...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET' and action == 'me':
        ...
        cur.execute(f"SELECT bio, favorite_genre, created_at, avatar_url, name_prefix, name_color, name_effect, badge_text, badge_effect, custom_role, hide_role FROM {SCHEMA}.users WHERE id = %s", (user['id'],))
        row = cur.fetchone()
        cur.close(); conn.close()
        return ok({**user, 'bio': row[0] or '', 'favorite_genre': row[1] or '', 'created_at': str(row[2]), 'avatar_url': row[3] or '', 'name_prefix': row[4] or '', 'name_color': row[5] or '', 'name_effect': row[6] or '', 'badge_text': row[7] or '', 'badge_effect': row[8] or '', 'custom_role': row[9] or '', 'hide_role': bool(row[10]), **stats})
    ...
    if method == 'GET' and action == 'profile':
        ...
        cur.execute(f"SELECT id, username, role, bio, favorite_genre, created_at, avatar_url, name_prefix, name_color, name_effect, badge_text, badge_effect, custom_role, hide_role FROM {SCHEMA}.users WHERE username = %s AND status = 'active'", (username,))
        row = cur.fetchone()
        if not row:
            cur.close(); conn.close()
        ...
        return ok({
            ...
            'custom_role': row[12] or '', 'hide_role': bool(row[13]),
            **stats
        })
    ...
    if method == 'GET' and action == 'users':
        ...
        cur.execute(f"SELECT id, username, email, role, status, created_at, name_color, name_effect, badge_text, badge_effect, ban_reason, custom_role, hide_role FROM {SCHEMA}.users ORDER BY created_at DESC")
        rows = cur.fetchall()
        cur.close(); conn.close()
        return ok([{'id': r[0], 'username': r[1], 'email': r[2], 'role': r[3], 'status': r[4], 'created_at': r[5], 'name_color': r[6] or '', 'name_effect': r[7] or '', 'badge_text': r[8] or '', 'badge_effect': r[9] or '', 'ban_reason': r[10] or '', 'custom_role': r[11] or '', 'hide_role': bool(r[12])} for r in rows])
    ...
    if method == 'POST':
        ...
        if action == 'update_user':
            ...
            ALLOWED_BADGE_EFFECTS = ['','badge-blood','badge-gold','badge-ice','badge-void','badge-fire','badge-poison','badge-ghost','badge-crimson','badge-eldritch','badge-glitch','badge-shadow']
            ...
            if new_badge_text is not None:
                badge_val = new_badge_text.strip()[:40] if new_badge_text else None
            ...
            new_hide_role = body.get('hide_role')
            if new_hide_role is not None:
                cur.execute(f"UPDATE {SCHEMA}.users SET hide_role = %s WHERE id = %s", (bool(new_hide_role), target_id))
            conn.commit()
            # Возвращаем обновлённые данные пользователя
            cur.execute(f"SELECT name_color, name_effect, badge_text, badge_effect, custom_role, hide_role FROM {SCHEMA}.users WHERE id = %s", (target_id,))
            row = cur.fetchone()
            cur.close(); conn.close()
            return ok({'success': True, 'name_color': row[0] or '' if row else '', 'name_effect': row[1] or '' if row else '', 'badge_text': row[2] or '' if row else '', 'badge_effect': row[3] or '' if row else '', 'custom_role': row[4] or '' if row else '', 'hide_role': bool(row[5]) if row else False})
